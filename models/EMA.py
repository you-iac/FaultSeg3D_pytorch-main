...
class EMA(nn.Module):  # 定义一个继承自 nn.Module 的 EMA 类
    ...
    def forward(self, x):  # 定义前向传播函数
        ...
        x_l = self.pool_w(group_x).permute(0, 1, 4, 2, 3)  #32 1 h  w 1

        hw = self.conv1x1x1(torch.cat([x_h, x_w, x_l], dim=3))  # 将池化结果拼接并通过 1x1 卷积层
        x_h, x_w, x_l  = torch.split(hw, [h, w, l], dim=3)  # 将卷积结果按高度和宽度长度
