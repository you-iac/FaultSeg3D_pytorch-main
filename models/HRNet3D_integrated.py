# ------------------------------------------------------------------------------
# HRNet3D Integrated - 3D High-Resolution Network for 3D Fault Segmentation
# 集成版本：将所有HRNet3D功能整合到一个文件中
# ------------------------------------------------------------------------------
...
class HighResolutionNet3D(nn.Module):
    def __init__(self, n_channels=1, n_classes=2, config='fault_seg', **kwargs):
        ...
        self.stage4_cfg = {
            ...
            'NUM_BLOCKS': [4, 4, 4, 4],
            'NUM_BRANCHES': 4,
            'NUM_MODULES': stage4_modules,
            'FUSE_METHOD': 'SUM'
        }

        # stem net - 3D版本
        self.conv1 = nn.Conv3d(n_channels, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
    ...
    def forward(self, x):
        ...
        x1 = F.interpolate(x[1], size=(x0_d, x0_h, x0_w), mode='trilinear', align_corners=True)
        x2 = F.interpolate(x[2], size=(x0_d, x0_h, x0_w), mode='trilinear', align_corners=True)
        x3 = F.interpolate(x[3], size=(x0_d, x0_h, x0_w), mode='trilinear', align_corners=True)
        ...
        # 上采样到原始输入尺寸
        x = F.interpolate(x, size=(128, 128, 128), mode='trilinear', align_corners=True)
...
# 为了与现有框架兼容，创建FaultSeg3D类
class FaultSeg3D(nn.Module):
    def __init__(self, n_channels, n_classes, config='fault_seg'):
        super(FaultSeg3D, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        
        # 使用HRNet3D作为backbone
        self.hrnet = HighResolutionNet3D(n_channels, n_classes, config)
...
if __name__ == '__main__':
    ...
    # 测试不同配置
    configs = ['fault_seg', 'hrnet_w18', 'hrnet_w32', 'hrnet_w48']
    
    for config in configs:
        print(f"\n测试配置: {config}")
        net = FaultSeg3D(1, 2, config).to(device)
