...
class FaultSeg3D(nn.Module):
    """
    结合DCN可变形卷积和轴向注意力的3D UNet
    保持原有结构，在关键层添加轴向注意力机制
    """
...
#
# 模型特点：
# 1. 使用DCN可变形卷积增强特征提取能力
# 2. 添加轴向注意力机制，降低计算复杂度
# 3. 保持原有UNet结构，只在关键层添加注意力
#
# 轴向注意力优势：
# - 复杂度从O(N²)降低到O(3×N×sqrt(N))
# - 对于128³特征图，显存需求从4.4万亿降低到8亿
# - 可以在大尺寸特征图上使用
#
# 注意力层分布：
# - 64×64×64层：32通道轴向注意力
# - 32×32×32层：64通道轴向注意力
# - 16×16×16层：128通道轴向注意力
# ================================================================