...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels=32, out_channels=32):
        ...
        self.e1 = DoubleConv(in_channels, out_channels)

        # E2/E3: 编码器第二阶段/瓶颈层 (编码器最终输出FE3) - 使用DoubleConv增强特征提取
        self.e2 = DoubleConv(out_channels, out_channels)

    def forward(self, x):
        """
        x: (B, 32, 32, 32, 32) - 输入模块输出
        return:
            F_E1: (B, 32, 32, 32, 32) - 编码器第一阶段输出（用于跳跃连接）
            F_E3: (B, 32, 32, 32, 32) - 编码器瓶颈层输出
        """
        F_E1 = self.e1(x)  # 第一阶段输出
        F_E3 = self.e2(F_E1)  # 第二阶段输出（瓶颈层）
        return F_E1, F_E3
...
class ForwardDecoder(nn.Module):
    ...
    解码器内部的卷积块通道数应与编码器保持一致(32)，但最终输出被设计为16通道。
    使用编码器的浅层特征 F_E1 作为跳跃连接。
    """

    def __init__(self, in_channels=32, out_channels=16):
        super().__init__()

        # 解码器内部保持32通道（与编码器一致）- 使用DoubleConv增强特征提取
        self.block1 = DoubleConv(in_channels, in_channels)  # 32 → 32
        self.block2 = DoubleConv(in_channels, in_channels)  # 32 → 32

        # 跳跃连接融合层（融合F_E1和当前特征）- 使用DoubleConv增强特征提取
        self.fusion = DoubleConv(in_channels * 2, in_channels)  # 32+32 → 32

        # 最终输出层：32 → 16 - 使用DoubleConv增强特征提取
        self.final_conv = DoubleConv(in_channels, out_channels)

    def forward(self, f_e3, f_e1):
        """
        f_e3: (B, 32, 32, 32, 32) - 编码器瓶颈特征 F_E3
        f_e1: (B, 32, 32, 32, 32) - 编码器第一阶段特征 F_E1（用于跳跃连接）
        return: (B, 16, 32, 32, 32)
        """
        # 从F_E3开始解码
        x = self.block1(f_e3)  # (B, 32, 32, 32, 32)
        x = self.block2(x)  # (B, 32, 32, 32, 32)

        # 与F_E1进行跳跃连接融合
        x = torch.cat([x, f_e1], dim=1)  # (B, 64, 32, 32, 32)
        x = self.fusion(x)  # (B, 32, 32, 32, 32)
...
class BackwardDecoder(nn.Module):
    ...
    结构上与Forward Decoder类似，但权重独立，旨在提取与FD互补的信息。
    解码器内部的卷积块通道数应与编码器保持一致(32)，但最终输出被设计为16通道。
    使用编码器的浅层特征 F_E1 作为跳跃连接。
    """
...
class OutputModule(nn.Module):
    ...
    def __init__(self, n_classes=2):
        ...
        # 三个输入：FD (16) + BD (16) + Proj F_E3 (16) = 48 通道
        in_channels = 48

        # 将 32 通道的 F_E3 降维到 16 通道 - 使用DoubleConv增强特征提取
        self.encoder_proj = DoubleConv(32, 16)

        # 第一次上采样: 32³ → 64³ (通道: 48 → 24) - 使用trilinear插值，参考faultseg3d_.py
        self.up1 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='trilinear', align_corners=True),
            DoubleConv(in_channels, in_channels // 2)  # 48 → 24
        )

        # 第二次上采样: 64³ → 128³ (通道: 24 → 12) - 使用trilinear插值
        self.up2 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='trilinear', align_corners=True),
            DoubleConv(in_channels // 2, 12)  # 24 → 12
        )

        # 最终输出层: 1×1×1卷积，输出n_classes个通道 (12 → n_classes)
        self.final_conv = nn.Conv3d(12, n_classes, kernel_size=1)

    def forward(self, forward_out, backward_out, encoder_bottleneck):
        """
        forward_out: (B, 16, 32, 32, 32)
        backward_out: (B, 16, 32, 32, 32)
        encoder_bottleneck: (B, 32, 32, 32, 32) - F_E3
        """

        # 1. 将编码器瓶颈特征降维到 16 通道
        proj_bottleneck = self.encoder_proj(encoder_bottleneck)  # (B, 16, 32, 32, 32)

        # 2. 拼接三个特征图 (16+16+16 = 48 通道)
        fused = torch.cat([forward_out, backward_out, proj_bottleneck], dim=1)  # (B, 48, 32, 32, 32)

        # 3. 两次上采样: 32³ → 64³ → 128³
        x = self.up1(fused)  # 32³ → 64³ (48 → 24)
        x = self.up2(x)  # 64³ → 128³ (24 → 12)
...
class FaultSeg3D(nn.Module):
    ...
    def __init__(self, in_channels=1, n_classes=2, base_channels=32):
        ...
        # 2. 编码器: 32³ → 32³ (输出F_E1和F_E3)
        self.encoder = Encoder(base_channels, base_channels)  # 输出 F_E1 和 F_E3，都是 32 通道
    ...
    def forward(self, x):
        ...
        # 2. 编码器: 输出F_E1和F_E3
        F_E1, F_E3 = self.encoder(x_input)  # 两者都是 (B, 32, 32, 32, 32)

        # 3. 双解码器: 并行处理 F_E3，使用F_E1作为跳跃连接
        forward_out = self.forward_decoder(F_E3, F_E1)  # (B, 16, 32, 32, 32)
        backward_out = self.backward_decoder(F_E3, F_E1)  # (B, 16, 32, 32, 32)

        # 4. 输出模块: 融合并恢复到 128³
        logits = self.output_module(forward_out, backward_out, F_E3)  # (B, n_classes, 128, 128, 128)
