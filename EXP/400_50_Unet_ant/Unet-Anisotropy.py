...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None, kernel_size=(3, 3, 3), padding=(1, 1, 1)):
        super().__init__()
        if not mid_channels:
...
class FaultSeg3D(nn.Module):
    def __init__(self, n_channels, n_classes):
        ...
        # 第一层使用3x3x7卷积，padding为(1,1,3)保持空间尺寸不变
        self.inc = DoubleConv(n_channels, 16, kernel_size=(3, 3, 7), padding=(1, 1, 3))

        # 第二层使用3x3x7卷积，padding为(1,1,3)保持空间尺寸不变
        self.down1 = Down(16, 32, kernel_size=(3, 3, 7), padding=(1, 1, 3))

        # 第三层使用3x3x5卷积，padding为(1,1,2)保持空间尺寸不变
        self.down2 = Down(32, 64, kernel_size=(3, 3, 5), padding=(1, 1, 2))

        # 第四层使用3x3x5卷积，padding为(1,1,2)保持空间尺寸不变
        self.down3 = Down(64, 128, kernel_size=(3, 3, 5), padding=(1, 1, 2))
...
# ================================================================
# Total params: 2,079,058
# Trainable params: 2,079,058
# Non-trainable params: 0
# ----------------------------------------------------------------
# Input size (MB): 8.00
# Forward/backward pass size (MB): 5962.00
# Params size (MB): 7.93
# Estimated Total Size (MB): 5977.93
# ----------------------------------------------------------------
